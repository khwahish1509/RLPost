...
- [x] **Confirmation at larger n (2026-07-24):** trained step-29 re-evaluated on 12 held-out streams → **Lift 1.000 on every stream** (eval #24, 0 errors); untrained baseline independently re-confirmed at ~0.56 over 12 streams mid-run. The headline is now **base ~0.55 → trained 1.000**, measured twice on each side.
- [x] **S3 — CONVERSATIONAL MEMORY (2026-07-29):** the jump from arithmetic to language. New `environments/memory_stream`: multi-session user dialogues with stable facts, updates (latest value wins), chatter, and abstention traps (other people's attributes); frozen reader answers from the notebook alone; Lift = accuracy with − without notes. Calibration: untrained Qwen scores **−0.028** (eval #32 — its notes are *worse than nothing*: keeps traps and chatter, drops facts). Trained 30 GRPO steps on a free T4 (completed inside the wall, curve −0.02 → 0.50 clean, run #5, ckpts every 5). **Held-out verdict (eval #33, n=12, 0 errors): Lift −0.028 → +0.486** — reader accuracy 0.167 → 0.653 from the trained notebooks. Honest mechanism note: the learned strategy is *verbatim retention* (beats fact-dropping, still keeps traps; the 600-char cap barely binds at 4 sessions), so traps/updates still cost it — the remaining headroom to ~0.83 is the selection skill.
- [x] **S3b — SELECTION FORCED, AND LEARNED (2026-07-30).** Curriculum: 6 sessions · 4 distractors/session · 2 updates vs a **350-char cap** — a verbatim dump overflows 3.6×, so S3's copy-everything strategy fails by construction. Trained 30/30 steps on a free T4 (completed inside the wall; curve −0.156 pre-flight → 0.396 final, flat for ~20 steps then a clean takeoff, still climbing at the end, no collapse; ckpts every 5). **Three-way held-out verdict, n=10, same reader (evals #35 · #37 · #38): S3b +0.367 · S3 copier +0.100 · untrained −0.133.** The copier's collapse (from +0.486 on its own loose-cap curriculum to +0.100 here) confirms the S3 diagnosis: copying was its whole trick.
- [x] **Mechanism, measured (not asserted).** Per-question-type accuracy with the cap applied as the reader sees it: **facts** — S3b 0.87 vs copier 0.52 (truncated in 10/10 streams) vs untrained 0.06; **updates** — S3b 0.26 vs 0.00 for both others; **abstention** — 0.00 for all three. Notebook size: S3b writes 221 chars (0/10 over cap) vs the copier's 764 (10/10 truncated). The learned skill is **compression**: prose is rewritten into dense `Key: Value` pairs, and updates are sometimes encoded explicitly ("Matcha: Not. Cocoa: Present."). What it did **not** learn: dropping other people's details (every abstention trap survives) and reliably deleting stale values (it often keeps both cities). Honest summary: *fit-the-budget by compressing* — a real, transferable skill, but not yet *choose-what-matters*.
- [x] **S3c — SELECTION LEARNED (2026-08-01). The three-stage arc is complete.** Warm-started from the S3b compression checkpoint (new trainer feature: `init_adapter`), trained 30/30 steps on the selection-priced curriculum (2 recurring traps/stream, abstention+update questions weighted 2×). Held-out weighted verdict (evals #42/#43/#44 vs #39): **S3c final +0.375 · S3c step-24 +0.227 · S3b incumbent −0.023 · untrained −0.182** (reader accuracy 0.739 vs 0.341 vs 0.182). Final checkpoint IS the best this time.
- [x] **The mechanism, measured — the headline table of the project.** Same 8 held-out streams, cap applied as the reader sees it: **traps written into the notebook: S3c 3/16 vs S3b 13/16 vs untrained 8/16. Stale values kept: 2/16 vs 9/16 vs 6/16.** Abstention accuracy 0.81 vs 0.19; update accuracy 0.75 vs 0.19. The trade: facts dipped 0.75 → 0.62 (being choosy occasionally drops a real fact) — a price worth paying under any reasonable weighting. The model now *keeps the user's facts, refuses other people's details, and deletes outdated values* — retention → compression → **selection**, each stage forced by curriculum design and verified by reading notebooks, not just scores.
- [x] **Real-reader check PASSED (2026-08-01).** The stored held-out notebooks from the three-way verdict, re-graded by grok instead of the deterministic checker (same frozen notebooks, same questions, T=0 cached): **S3c +0.352 · S3b +0.011 · untrained +0.091** — vs the checker's +0.375 / −0.023 / −0.182. The trained lift survives a real reader essentially unchanged (+0.375 → +0.352), and the selection model's margin stays decisive. Honest nuance worth keeping: a smart reader partially rescues *bad* notebooks (untrained −0.18 → +0.09, because grok reasons about attribution and often refuses traps the checker falls for) — which makes the S3c margin the conservative, defensible number.
- [x] **Cross-domain transfer test (2026-08-01) — the north-star answer, and it's nuanced.** A second generator (`worklogs.py`): standup-style work streams — projects, deadlines, clients, budgets; different phrasing; trap flavours are other teams' managers/vendors/off-sites. Zero retraining; same weighted rules; n=8 (evals #45/#46/#47): **S3c +0.159 · S3b +0.182 · untrained −0.159.** What the mechanism shows: (1) **the trained core transfers** — both trained models hold strong positive lift in a world they never saw, while untrained stays deeply negative; (2) **update discipline transfers** — S3c still deletes stale values out-of-domain (3/16 kept vs 2/16 at home; update accuracy 0.69); (3) **trap-refusal does NOT transfer** — S3c writes down 10/16 work-world traps vs 3/16 at home: its attribution filter appears keyed to personal-domain surface cues ("my neighbor's…", "my cousin's…") rather than fully general reasoning. Out-of-domain, S3c ≈ S3b within noise. Honest statement for the writeup: *structured retention and update-replacement are transferable skills; fine-grained attribution filtering, at 0.6B with single-domain training, is not.* Obvious future work: mixed-domain training.
- [ ] Validation remaining: LongMemEval/LoCoMo-subset taste · the writeup · tag v1.0.
...
- [x] **THE NORTH-STAR EXPERIMENT — ANSWERED (2026-07-24).** Trained (step-29) vs prompted (base) Scribe across three drift rungs, n=8 each, held-out streams, server identity verified before every eval: **rung A (reuse tags removed): 1.000 vs 0.536 · rung B (5 distractors vs trained-on 3): 1.000 vs 0.518 · rung C (12 tasks vs trained-on 8): 1.000 vs 0.443** (evals #25–27 trained, #28–30 base). **The trained skill transfers: the prompted model degrades with distance (0.55→0.44) while the trained Scribe holds 1.000 on every rung — and the gap widens (+0.45→+0.56).** Rung A is the decisive ablation: without the `(needed later)` labels it trained with, the Scribe still filters distractors by content — it learned selection, not tag-copying. Scope honestly stated: drift within the same task family (near-to-mid distance), fake Player, n=8/rung. A first mislabeled base run (surviving server on the old port answered for 'base') was caught by its impossible numbers and its trained-signature notebooks, deleted, and re-run with identity checks — the receipts discipline working as designed.
