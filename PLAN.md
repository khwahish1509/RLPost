...
- [ ] **S2 RUN (launchable now):** `nanolab train --cloud configs/qwen3-0.6b-scribe.toml` — env-aware kernel installs scribe_stream, trains 80 steps (lr 5e-5, per-decade checkpoints), auto-merges on pull.
- [ ] S2 check (after the run): trained small Scribe ≥ prompted same-size Scribe (0.548) on held-out Lift
